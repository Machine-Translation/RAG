...
def get_prop_level(level):
    ...
    if level == 1:
        ...
        prob_dic[note_to_int.get((0.5, True))] = 0.1      #eigth rest
        prob_dic[note_to_int.get((0.5, False))] = 0.1     #eigth note
        prob_dic[note_to_int.get((1.0, True))] = 0.05     #quarter rest
...
def create_rand_midi(level, file_name):
    ...
    rand_notes = np.random.choice(ids, p=distribution, size=50)

    # create note and chord objects based on the values generated by the model
    for id in rand_notes:
