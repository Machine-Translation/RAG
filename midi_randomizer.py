...
def create_example(level):
    """
    Create an example with a random tempo and random set up notes based on
        ...
        def set_note(element, is_rest):
            ...
            for np in note_parts:
                ...
                if num_whole_notes > 0:
                    notes.append(note_to_int.get((temp_length, is_rest)))
                    ...
                    #print(note_to_int.get((temp_length, is_rest)))
                    notes.append(note_to_int.get((temp_length, is_rest)))
